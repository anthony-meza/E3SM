...
# - if running off maint-1.2, update batch job submission settings in 
#   env_batch.xml
# - if running off master, update batch job submission settings in 
#   env_workflow.xml
# 
#####################################################################
...
export USER=ameza9
export PROJECT=double-precision
...
export REPO_NAME=maint-1.2.20190923 
# This is maint-1.2 with E3SM PR #3097 for updated machine files
# git cherry-pick a0e6ec283398dbfb48940af1de2837df302245c0 -X theirs
...
# ./xmlchange -file env_batch.xml -subgroup case.run -id JOB_WALLCLOCK_TIME -val "13:00:00"
#
...
#####################################################################
# If not running off maint-1.2, update env_workflow.xml instead of env_batch.xml
# ./xmlchange -file env_workflow.xml -subgroup case.run -id JOB_QUEUE -val debug
